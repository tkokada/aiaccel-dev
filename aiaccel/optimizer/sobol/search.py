...
class SobolOptimizer(AbstractOptimizer):
    ...
    ToDo: The development of original library was stopped. It's recommended to
        ...
        Make it clear to resume this optimizer with Sobol sampler. Current code resume the sampler counts with a number
        of FINISHED PARAMETER FILES. Confirm whether the current code resumes for any timings of quits.
    """
    ...
    def pre_process(self) -> None:
        ...
        # SciPy Sobol
        if SCIPY_SOBOL:
            n_params = len(self.params.get_parameter_list())
            self.sampler = qmc.Sobol(d=n_params, scramble=False)
